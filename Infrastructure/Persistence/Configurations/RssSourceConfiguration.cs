    ...
    public class RssSourceConfiguration : IEntityTypeConfiguration<RssSource>
    {
        public void Configure(EntityTypeBuilder<RssSource> builder)
        {
            ...
            _ = builder.Property(rs => rs.SourceName)
                .IsRequired()
                .HasMaxLength(150);
            _ = builder.HasIndex(rs => rs.SourceName)
                .HasDatabaseName("IX_RssSources_SourceName"); // Good for admin UI searches.

            // --- Properties with Database-Generated Default Values ---
            _ = builder.Property(rs => rs.IsActive)
                .IsRequired()
                .HasDefaultValue(true); // New sources should be active by default.

            _ = builder.Property(rs => rs.CreatedAt)
                .IsRequired()
                .HasDefaultValueSql("NOW()"); // For PostgreSQL. Use GETUTCDATE() for SQL Server.

            _ = builder.Property(rs => rs.FetchErrorCount)
                .IsRequired()
                .HasDefaultValue(0); // New sources start with zero errors.

            // --- Nullable & Other Properties ---
            _ = builder.Property(rs => rs.UpdatedAt);
            _ = builder.Property(rs => rs.LastModifiedHeader).HasMaxLength(100);
            _ = builder.Property(rs => rs.ETag).HasMaxLength(255);
            _ = builder.Property(rs => rs.LastFetchAttemptAt);
            _ = builder.Property(rs => rs.LastSuccessfulFetchAt);
            _ = builder.Property(rs => rs.FetchIntervalMinutes);
            _ = builder.Property(rs => rs.Description).HasMaxLength(1000);
            _ = builder.Property(rs => rs.DefaultSignalCategoryId);
            ...
            _ = builder.HasIndex(rs => rs.IsActive)
            ...
            _ = builder.HasIndex(rs => rs.LastFetchAttemptAt)
            ...
            _ = builder.HasIndex(rs => new { rs.IsActive, rs.LastFetchAttemptAt })
            ...
            _ = builder.HasMany(rs => rs.NewsItems)
            ...
            _ = builder.HasOne(rs => rs.DefaultSignalCategory)
