...
namespace Application.Services
{
    public class GeminiService : IGeminiService
    {
        ...
        // --- START OF CORRECTION AND ENHANCEMENT ---
        private async Task<string?> AttemptApiCallAsync(AiApiConfiguration config, string message, CancellationToken cancellationToken)
        {
            ...
            {
                // Catch network errors or client-side timeouts and trigger a failover.
            ...
            // --- Robust handling for ALL non-success status codes ---
            // Any non-2xx response from the API will be treated as a reason to fail over to the next configuration.
            var errorContent = await response.Content.ReadAsStringAsync(cancellationToken);
            _logger.LogWarning(
                "Gemini API call failed for config Id: {ConfigId}, ApiKeyName: {ApiKeyName}. Status: {StatusCode}. Response: {ErrorContent}",
                config.Id,
                config.ApiKeyName,
                response.StatusCode,
                errorContent);

            // Throw the specific exception Polly is waiting for to trigger the next fallback.
            throw new GeminiApiFailoverException($"API call for config {config.Id} failed with status {response.StatusCode}.");
        }
        // --- END OF CORRECTION AND ENHANCEMENT ---
