...
namespace Application.Services
{
    public class NotificationDispatchService : INotificationDispatchService
    {
       ...
       INotificationRateLimiter rateLimiter)
        ...
        private async Task DispatchSingleNewsItemInternalAsync(NewsItem newsItem, CancellationToken cancellationToken)
        {
            const int chunkSize = 500;
            List<long> uniqueTelegramIds = new List<long>(); // Initialize here

            // 1. Fetch users for this news item.
            var targetUsers = await _userRepository.GetUsersForNewsNotificationAsync(
                newsItem.AssociatedSignalCategoryId, newsItem.IsVipOnly, cancellationToken);

            uniqueTelegramIds = targetUsers
                .Select(u => long.TryParse(u.TelegramId, out long id) ? (long?)id : null)
                .Where(id => id.HasValue)
                .Select(id => id.Value)
                .Distinct()
            ...
            if (!uniqueTelegramIds.Any())
            {
                _logger.LogInformation("No eligible users found for NewsItem {NewsItemId}. Skipping.", newsItem.Id);
                return;
            }

            // 2. Filter users by their individual rate limit.
            List<long> eligibleUsersForThisDispatch = new List<long>();
            const int globalNotificationLimit = 5;
            TimeSpan globalNotificationPeriod = TimeSpan.FromMinutes(15);
            ...
            foreach (long userId in uniqueTelegramIds)
            {
                // This check is now extremely effective because the orchestrator is serialized.
                if (await _rateLimiter.IsUserOverLimitAsync(userId, globalNotificationLimit, globalNotificationPeriod))
                {
                    _logger.LogInformation("Dispatch Filter: Skipping user {UserId} for NewsItem {NewsItemId} (rate limit).", userId, newsItem.Id);
                }
                else
                {
                    eligibleUsersForThisDispatch.Add(userId);
            ...
            if (!eligibleUsersForThisDispatch.Any())
            {
                _logger.LogInformation("No users remaining for NewsItem {NewsItemId} after rate filtering.", newsItem.Id);
                return;
            }

            // 3. Cache the final, filtered user list to Redis.
            string userListCacheKey = $"dispatch:users:{newsItem.Id}";
            string serializedUserIds = JsonSerializer.Serialize(eligibleUsersForThisDispatch);
            await _redisDb.StringSetAsync(userListCacheKey, serializedUserIds, TimeSpan.FromHours(24));
            _logger.LogInformation("Cached {Count} user IDs to Redis for NewsItem {NewsItemId}.", eligibleUsersForThisDispatch.Count, newsItem.Id);

            // 4. Enqueue the chunk manager jobs.
            int totalUsers = eligibleUsersForThisDispatch.Count;
            int chunks = (int)Math.Ceiling((double)totalUsers / chunkSize);
            for (int i = 0; i < chunks; i++)
            {
                ...
                // This enqueues the job that will then use `Schedule` to stagger the final sends.
                _jobScheduler.Enqueue<INotificationDispatchService>(
                    service => service.ProcessNotificationChunkAsync(newsItem.Id, userListCacheKey, chunkStartIndex, itemsInThisChunk, CancellationToken.None));
            }

            _logger.LogInformation("Enqueued {ChunkCount} chunk managers for NewsItem {NewsItemId}.", chunks, newsItem.Id);
        }
