...
public static class GeminiErrors
{
    ...
    public static ResilientResponse<T> InvalidArgument<T>(string details) =>
        ResilientResponse<T>.CreateNonRetryableError(400, $"Invalid Argument: The request is malformed. Details: {details}", "INVALID_ARGUMENT");
    ...
    public static ResilientResponse<T> FailedPrecondition<T>() =>
        ResilientResponse<T>.CreateNonRetryableError(400, "Failed Precondition: Gemini API free tier not available or billing not enabled.", "FAILED_PRECONDITION");
    ...
    public static ResilientResponse<T> PermissionDenied<T>() =>
        ResilientResponse<T>.CreateNonRetryableError(403, "Permission Denied: The API key is invalid or lacks necessary permissions.", "PERMISSION_DENIED");
    ...
    public static ResilientResponse<T> NotFound<T>(string resourceDetails) =>
        ResilientResponse<T>.CreateNonRetryableError(404, $"Not Found: The requested resource was not found. Details: {resourceDetails}", "NOT_FOUND");
    ...
    public static ResilientResponse<T> ResourceExhausted<T>(int retryAfterSeconds = 60) =>
        ResilientResponse<T>.CreateRetryableError(429, "Resource Exhausted: Rate limit exceeded. Please wait and retry.", "RESOURCE_EXHAUSTED", retryAfterSeconds);
    ...
    public static ResilientResponse<T> InternalError<T>(int retryAfterSeconds = 30) =>
        ResilientResponse<T>.CreateRetryableError(500, "Internal Server Error: An unexpected error occurred on Google's side. Please retry.", "INTERNAL", retryAfterSeconds);
    ...
    public static ResilientResponse<T> ServiceUnavailable<T>(int retryAfterSeconds = 60) =>
        ResilientResponse<T>.CreateRetryableError(503, "Service Unavailable: The service is temporarily overloaded. Please retry.", "UNAVAILABLE", retryAfterSeconds);
    ...
    public static ResilientResponse<T> DeadlineExceeded<T>(int retryAfterSeconds = 30) =>
        ResilientResponse<T>.CreateRetryableError(504, "Deadline Exceeded: The request timed out. Please retry.", "DEADLINE_EXCEEDED", retryAfterSeconds);
}